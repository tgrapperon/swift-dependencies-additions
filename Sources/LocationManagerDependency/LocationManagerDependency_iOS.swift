  ...
  extension LocationManager {
    static var system: Self {
      ...
      let _implementation = Implementation(
        ...
        showsBackgroundLocationIndicator: .init(
          .init(
            get: { manager.showsBackgroundLocationIndicator },
            set: { manager.showsBackgroundLocationIndicator = $0 }
          )),
        activityType: .init(
          .init(
        ...
        startMonitoringLocationPushes: .init {
          #if !targetEnvironment(macCatalyst)
            if #available(iOS 15.0, *) {
              return manager.startMonitoringLocationPushes(completion: $0)
            } else {
              fatalError("'startMonitoringLocationPushes(completion:)' is unavailable")
            }
          #else
            fatalError("'startMonitoringLocationPushes(completion:)' is unavailable")
          #endif
        },
        stopMonitoringLocationPushes: .init { () -> Void in
          #if !targetEnvironment(macCatalyst)
            if #available(iOS 15.0, *) {
              return manager.stopMonitoringLocationPushes()
            } else {
              fatalError("'stopMonitoringLocationPushes()' is unavailable")
            }
          #else
            fatalError("'stopMonitoringLocationPushes()' is unavailable")
          #endif
    ...
    static var unimplemented: LocationManager {
      let _implementation = Implementation(
        ...
        allowsBackgroundLocationUpdates: .unimplemented(
          ...
          placeholder: false),
        showsBackgroundLocationIndicator: .unimplemented(
          #"@Dependency(\.locationManager.showsBackgroundLocationIndicator)"#),
        ...
        startMonitoringLocationPushes: .unimplemented(
          #"@Dependency(\.locationManager.startMonitoringLocationPushes)"#),
        stopMonitoringLocationPushes: .unimplemented(
          #"@Dependency(\.locationManager.stopMonitoringLocationPushes)"#),
